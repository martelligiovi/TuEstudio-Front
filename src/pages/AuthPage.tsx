              ...
              <div className="mt-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className="flex justify-center items-center h-[40px] border border-hairline rounded-md bg-surface-soft hover:bg-surface-cream-strong transition-colors font-sans text-button gap-2"
                  >
          ...
          >
            <div className="w-full max-w-sm">
              ...
              <div className="mt-5">
                ...
                <div className={`grid grid-cols-2 gap-3 transition-opacity duration-200 ${!role ? 'opacity-40 pointer-events-none' : ''}`}>
                  <button
                    type="button"
                    disabled={!role}
                    title={!role ? 'Seleccioná tu perfil primero' : undefined}
                    onClick={() => role && initiateOAuth('google', role === 'student' ? 'STUDENT' : 'TUTOR')}
                    className="flex justify-center items-center h-[40px] border border-hairline rounded-md bg-surface-soft font-sans text-button gap-2 disabled:cursor-not-allowed hover:bg-surface-cream-strong transition-colors"
                  >
                    <svg className="w-4 h-4" viewBox="0 0 24 24">
                      <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
                      <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
                  ...
                  <button
                    ...
                    disabled={!role}
                    title={!role ? 'Seleccioná tu perfil primero' : undefined}
                    onClick={() => role && initiateOAuth('linkedin', role === 'student' ? 'STUDENT' : 'TUTOR')}
                    className="flex justify-center items-center h-[40px] border border-hairline rounded-md bg-surface-soft font-sans text-button gap-2 disabled:cursor-not-allowed hover:bg-surface-cream-strong transition-colors"
                  >
                    <svg className="w-4 h-4 text-[#0A66C2]" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
