...
export default function TeacherDashboard() {
	...
	async function fetchRequests(silent = false) {
		...
		try {
			...
			if (!silent)
				setError("No pudimos cargar tus solicitudes. Probá de nuevo.");
		} finally {
	...
	async function handleAttend(id: string) {
		...
		setRequests((prev) =>
			prev.map((r) =>
				r.id === id ? { ...r, status: "ATTENDED" as const } : r,
			),
		);
			...
			<main className="max-w-content mx-auto px-6 py-8">
					...
					>
						...
						<div className="flex-1">
							...
							<p className="font-sans text-body-sm text-muted mt-0.5">
								Te{" "}
								{profile.missingForActivation.length === 1 ? "falta" : "faltan"}{" "}
								{profile.missingForActivation.length}{" "}
								{profile.missingForActivation.length === 1 ? "campo" : "campos"}{" "}
				...
				{!loading && !error && requests.length === 0 && (
					<div className="flex flex-col items-center gap-4 py-16 text-center bg-surface-soft rounded-xl">
						<span className="material-symbols-outlined text-muted text-[56px]">
							inbox
						</span>
						<div>
							<p className="font-serif text-display-sm text-ink mb-1">
								Tu inbox está listo
							</p>
							<p className="font-sans text-body-md text-muted max-w-xs mx-auto">
								Tu perfil está activo. Las solicitudes de alumnos aparecerán
								aquí.
							</p>
						</div>
					</div>
